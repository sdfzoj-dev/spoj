		...
		if (isProblemVisibleToUser($problem, $myUser)) {
			...
			if (isset($_COOKIE['show_tags_mode'])) {
				foreach (queryProblemTags($problem['id']) as $tag) {
					$pos=strpos($tag,".");
					if($pos) {
						echo '<a class="uoj-problem-tag badge badge-'.substr($tag,0,$pos).'">', HTML::escape($tag), '</a>';
					}
					else {
						echo '<a class="uoj-problem-tag badge badge-primary">', HTML::escape($tag), '</a>';
					}
				}
			}
			echo '</td>';
			if (isset($_COOKIE['show_submit_mode'])) {
				...
				<td>
					<div class="progress bot-buffer-no">
						<div class="progress-bar progress-bar-success bg-danger" role="progressbar" aria-valuenow="$perc" aria-valuemin="0" aria-valuemax="100" style="width: $perc%; min-width: 20px;">{$perc}%</div>
					</div>
				</td>
EOD;
			}
	...
	$header = '<tr>';
	$header .= '<th class="text-center" style="width:5em;">'.UOJLocale::get('problems::status').'</th>';
