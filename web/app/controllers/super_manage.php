<?php
	...
	$user_pending_form = new UOJForm('user_pending');
	$user_pending_form->addInput('username', 'text', '用户名', '',
		function ($username) {
			if (!validateUsername($username)) {
				return '用户名不合法';
			}
			if (!DB::query("select * from user_pending where username='$username'")) {
				return '用户不存在';
			}
			return '';
		},
	...
	$user_pending_form->addSelect('op-type', $options, '操作类型', '');
	$user_pending_form->addVTextArea('explaination', '说明', '',function($x){return '';},null);
	$user_pending_form->handle = function() {
		global $user_pending_form;
		
		$username = $_POST['username'];
		$explaination=$_POST['explaination'];
		switch ($_POST['op-type']) {
			case 'accept':
                $user_info=DB::selectFirst("select * from user_pending where username='$username'", MYSQLI_ASSOC);
                DB::query("delete from user_pending where username = '$username'");
                DB::query("insert into user_info (username, email, password, svn_password, register_time) values ('".$user_info["username"]."', '".$user_info["esc_email"]."', '".$user_info["password"]."', '".$user_info["svn_password"]."', '".$user_info["register_time"]."')");

                $html = <<<EOD
<base target="_blank" />
<p>{$username}您好，您在SDFZ Public Online Judge的注册请求审核已通过^w^。如果不是您本人操作，请忽略。</p>
<p>说明 : {$explaination}</p>
<style type="text/css">
...
EOD;
                ...
                $mailer->addAddress($user_info['email'], $username);
                $mailer->Subject = $oj_name_short."审核通过";
                $mailer->msgHTML($html);
                $mailer->send();
        	...
        	case 'refuse':
                ...
                $html = <<<EOD
<base target="_blank" />
<p>{$username}您好，您在SDFZ Public Online Judge的注册请求审核未通过，请完善验证信息再次注册或联系管理员。如果不是您本人操作，请忽略。</p>
<p>说明 : {$explaination}</p>
<style type="text/css">
body{font-size:14px;font-family:arial,verdana,sans-serif;line-height:1.666;padding:0;margin:0;overflow:auto;white-space:normal;word-wrap:break-word;min-height:100px}
pre {white-space:pre-wrap;white-space:-moz-pre-wrap;white-space:-pre-wrap;white-space:-o-pre-wrap;word-wrap:break-word}
...
EOD;
                ...
                $mailer = UOJMail::noreply();
                $mailer->addAddress($user_info['email'], $username);
                $mailer->Subject = $oj_name_short."审核未通过";
                $mailer->msgHTML($html);
                $mailer->send();
...
EOD;
	...
	$userpendinglist_cols = array('username','email','register_time','validation_message');
    $userpendinglist_config = array();
    $userpendinglist_header_row = <<<EOD
    <tr>
        <th>用户名</th>
        <th>邮箱</th>
        <th>注册时间</th>
...
EOD;
    $userpendinglist_print_row = function($row) {
        echo <<<EOD
            <tr>
                <td>{$row['username']}</td>
                <td>{$row['email']}</td>
...
EOD;
	...
	$tabs_info = array(
		...
		'users-pending' => array(
			'name' => '用户审核',
			'url' => "/super-manage/users-pending"
		),
		'blogs' => array(
			'name' => '博客管理',
...
<div class="row">
	...
	<div class="col-sm-9">
		...
		<?php elseif ($cur_tab === 'users-pending'): ?>
			<?php $user_pending_form->printHTML(); ?>
			<h3>待审核名单</h3>
			<?php echoLongTable($userpendinglist_cols, 'user_pending', "1=1", '', $userpendinglist_header_row, $userpendinglist_print_row, $userpendinglist_config) ?>
