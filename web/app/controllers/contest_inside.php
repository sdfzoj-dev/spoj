...
if (!validateUInt($_GET['id']) || !($contest = queryContest($_GET['id']))) {
	...
	if (!hasContestPermission(Auth::user(), $contest)) {
		/*if ($contest['cur_progress'] == CONTEST_NOT_STARTED) {
		 ...
		 if ($contest['cur_progress'] == CONTEST_NOT_STARTED) {
			...
			elseif (!hasRegistered(Auth::user(),$contest)) {
				header("Location: /contest/{$contest['id']}/register");
				die();
			}
                }
	...
	if (isSuperUser($myUser)) {
		if (CONTEST_IN_PROGRESS <= $contest['cur_progress'] && $contest['cur_progress'] <= CONTEST_TESTING) {
			...
			$start_test_form->handle = function() {
				...
				if($contest['cur_progress']>=CONTEST_PENDING_FINAL_TEST) DB::query("update contests set status = 'testing' where id = {$contest['id']}");
			};
			...
			if($contest['cur_progress']<CONTEST_PENDING_FINAL_TEST){
				$start_test_form->submit_button_config['text'] = '重测';
		...
		if ($contest['cur_progress'] >= CONTEST_TESTING || ($contest['extra_config']['contest_type']=='IOI' && $contest['cur_progress'] >= CONTEST_PENDING_FINAL_TEST) ) {
