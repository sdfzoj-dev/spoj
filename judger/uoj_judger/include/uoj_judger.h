...
const RunLimit RL_DEFAULT = RunLimit(1000, 256, 64);
const RunLimit RL_JUDGER_DEFAULT = RunLimit(600000, 1024, 128);
const RunLimit RL_CHECKER_DEFAULT = RunLimit(5000, 256, 64);
const RunLimit RL_INTERACTOR_DEFAULT = RunLimit(1000, 256, 64);
const RunLimit RL_VALIDATOR_DEFAULT = RunLimit(5000, 256, 64);
const RunLimit RL_MARKER_DEFAULT = RunLimit(5000, 256, 64);
const RunLimit RL_COMPILER_DEFAULT = RunLimit(15000, 512, 64);
...
RunSimpleInteractionResult run_simple_interation(
	...
	irpc.limit.real_time = rpc.limit.real_time = rpc.limit.time + irpc.limit.time + 1000;
...
RunCompilerResult compile_cpp14(const string &name, const string &path = work_path) {
        return run_compiler(path.c_str(),
                        "/usr/bin/gcc-9.3.0/bin/g++", "-o", name.c_str(), "-x", "c++", (name + ".code").c_str(), "-lm", "-O2", "-DONLINE_JUDGE", "-std=c++14", NULL);
}
RunCompilerResult compile_cpp17(const string &name, const string &path = work_path) {
        return run_compiler(path.c_str(),
                        "/usr/bin/gcc-11.2.0/bin/g++", "-o", name.c_str(), "-x", "c++", (name + ".code").c_str(), "-lm", "-O2", "-DONLINE_JUDGE", "-std=c++17", NULL);
}
...
RunCompilerResult compile_cpp14_with_implementer(const string &name, const string &path = work_path) {
        return run_compiler(path.c_str(),
                        "/usr/local/gcc-9.3.0/bin/g++", "-o", name.c_str(), "implementer.cpp", "-x", "c++", (name + ".code").c_str(), "-lm", "-O2", "-DONLINE_JUDGE", "-std=c++14", NULL);
}
RunCompilerResult compile_cpp17_with_implementer(const string &name, const string &path = work_path) {
        return run_compiler(path.c_str(),
                        "/usr/local/gcc-11.2.0/bin/g++", "-o", name.c_str(), "implementer.cpp", "-x", "c++", (name + ".code").c_str(), "-lm", "-O2", "-DONLINE_JUDGE", "-std=c++17", NULL);
}
